<?php 
	...
	if (!empty($_FILES["fileToUpload"]['tmp_name'])) {
		...
		$prof_initial = $_GET['prof_initial'];
		$academic_year = $_GET['academic_year'];
		$semester = $_GET['semester'];

		$enr_table_name = str_replace("-","_",'enroll_'.$academic_year.'_'.$semester) ;
	    ...
	    if ($uploadOk) {
			...
			for ($row = 1; $row <= $highestRow; $row++){ 
			    ...
			    if (is_numeric($rowData[0][1])) {
				    ...
				    $mergeExcelToDB = $init->mergeExcelToGradeIndependent("grading_tmp", $rowData[0][1], $subject_code, $prelim, $midterm, $prefinal, $final, $unique_code, $year_level, $academic_year, $semester, $section, $prof_initial, $course);
				    if ($mergeExcelToDB) {
				    	$counter+=1;
				    }
			    } else {
